      ...
      <div class="main-content">
        <section class="section">
          ...
          <div class="section-body">
            <a href="{{ route('selectProductType') }}" class="btn btn-primary"><i class="fas fa-plus"></i> {{__('admin.Add New')}}</a>
            <div class="row mt-4">
                <div class="col">
                  <div class="card">
                    <div class="card-body">
                        ...
                        <table class="table table-striped" id="dataTable">
                            ...
                            <tbody>
                                @foreach ($products as $index => $product)
                                    <tr>
                                        <td>{{ ++$index }}</td>
                                        <td><a href="{{ Route('editProduct',['id' => $product->id, 'lang_code' => 'en'])}}">{{ html_decode($product->name?? $product->book_title ) }}</a></td>
                                        @php
                                            $total_sold=App\Models\OrderItem::where('Product_id', $product->id)->get()->count();
                                        @endphp
                                        <td>{{ $total_sold }}</td>
                                        <td>{{ optional(optional($product->category)->catlangfrontend)->name }}</td>
                                        <td>
                                            @if($product->status == 1)
                                        ...
                                        <td>
                                        <a href="{{ route('editProduct',['id' => $product->id, 'lang_code' => 'en']) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                        @php
